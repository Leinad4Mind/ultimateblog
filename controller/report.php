...
class report
{
	...
	*/
	public function handle($blog_id, $id, $mode)
	{
		...
		{
			throw new \phpbb\exception\http_exception(404, $this->lang->lang('BLOG_REPORT_INVALID_MODE'));
		}

		# Check if the user has permission to report
		if (!$this->auth->acl_get('u_ub_report'))
		{
			throw new \phpbb\exception\http_exception(403, $this->lang->lang('BLOG_REPORT_NO_PERMISSION'));
		}
		...
		{
			throw new \phpbb\exception\http_exception(400, $this->lang->lang('BLOG_REPORT_FORM_INVALID'));
		}
