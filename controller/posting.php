...
class posting
{
	...
	public function handle($mode, $blog_id = 0)
	{
		...
		{
			throw new \phpbb\exception\http_exception(404, $this->lang->lang('BLOG_ERROR_INVALID_MODE'));
		}

		# Grab blog2edit data (for edit or delete)
		if ($edit || $delete)
		{
		...
		{
			throw new \phpbb\exception\http_exception(403, $this->lang->lang('BLOG_ERROR_CANT_ADD'));
		}
		if ($edit && ((!$this->auth->acl_get('u_ub_edit') || $this->user->data['user_id'] != $blog_to_edit['author_id']) && !$this->auth->acl_get('m_ub_edit')))
		{
			throw new \phpbb\exception\http_exception(403, $this->lang->lang('BLOG_ERROR_CANT_EDIT'));
		}
		if ($delete && ((!$this->auth->acl_get('u_ub_delete') || $this->user->data['user_id'] != $blog_to_edit['author_id']) && !$this->auth->acl_get('m_ub_delete')))
		{
			throw new \phpbb\exception\http_exception(403, $this->lang->lang('BLOG_ERROR_CANT_DELETE'));
		}
		...
		if ($submit || $preview)
		{
			...
			(strlen($blog_to_update['blog_description']) < 50 || strlen($blog_to_update['blog_description'] > 125)) ? $errors[] = $this->lang->lang('BLOG_ERROR_DESC_BOUNDS', strlen($blog_to_update['blog_description'])) : false;
	...
	public function misc($mode, $blog_id)
	{
		...
		switch ($mode)
		{
			...
			case 'editdelete':
				...
				{
					throw new \phpbb\exception\http_exception(403, $this->lang->lang('BLOG_ERROR_CANT_EDIT_DELETE'));
				}
