	...
	public function ub_settings()
	{
		...
		if ($submit)
		{
			...
			if (empty($errors))
			{
				...
				$order_id = array(0 => 7, 1 => 6, 2 => 5, 3 => 4, 4 => 3, 5 => 2, 6 => 1);
				...
				$this->config_text->set('ub_announcement_text', htmlspecialchars_decode($this->request->variable('ub_announcement_text', ''), ENT_COMPAT));
		...
		# Display block order
		$block_current_order_array = array();
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			{
				$this->template->assign_block_vars($block_part . '.cats', array(
					'ID'	=> $cat['category_id'],
		...
		$this->template->assign_vars(array(
			...
			'UB_ANNOUNCEMENT_TEXT'	=> $submit ? $this->request->variable('ub_announcement_text', '', true) : $this->config_text->get('ub_announcement_text'),
